...
var displayMap = function displayMap(locations) {
  ...
  var map = new mapboxgl.Map({
    container: 'map',
    // here we need an id where we can put our map  so we have #map in tour page so map will be shown there
  ...
  var bounds = new mapboxgl.LngLatBounds(); // this will be the area which will be displayed on map
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Popup().setLngLat(loc.coordinates).setHTML("<p> Day".concat(loc.day, " : ").concat(loc.description, "</p>")).addTo(map); //  extends map bounds to include current location
...
var showAlert = function showAlert(type, msg) {
  hideAlert();
  var markup = "<div class=\"alert alert--".concat(type, "\">").concat(msg, "</div>"); //  inside of body but at the beginning
        ...
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            if (result.data.status === 'success') {
              (0, _alerts.showAlert)('success', 'logged in successfully'); //  to reload to main page
            ...
            console.log(result); //  if it login successfully it will produce a jwt token  and a cookie also containing jwt token and this will help to build the complete authentication
